...
def main(stdscr):
	...
	##맨 밑 화면 첫번째 줄은 D-PAD를 이용하라는 문구, 두번째 줄은 특수 버튼 지정()
	stdscr.addstr(bot_screen_hight, 0, "Use D-PAD and A button to select".center(bot_screen_width-1, ' '), MAIN_COLOR_REVERSE)
	stdscr.addstr(bot_screen_hight+1, 0, "SHUT DOWN  ".rjust(bot_screen_width-1, ' '))

	##좌측 상단에 타임 로고 삽입
	drew_ASCIIart(stdscr, time_logo, 7, 11)
